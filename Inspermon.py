...
time.sleep(0.5)
comeco=str(input("Você deseja começar um novo jogo(n) ou carregar um pré-existente(pe)? "))
if comeco == "n":
	...
	while True:
		fazer=str(input("O que você vai fazer? (Passear(P), Dormir(D), Insperdex(I) ou Salvar(S)? ")).lower() #Escolha de ação
		time.sleep(1.0)
